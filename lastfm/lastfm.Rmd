...
# added_n %>% filter(Tier %in% 1:3) %>% sample_n(200) %>% select(1:4) %>% bind_rows(., jpa) %>% distinct() %>% write.csv("../ranker/jp_albums.csv")
...
arts <- added_n %>% filter(n >100) %>% pull(artist) %>% unique()

art_tags <- map(arts, fetch_artist_toptags) %>% list_rbind()

art_tags %>% group_by(tag) %>% mutate(n = n()) %>% filter(n >5)

art_tags %>% count(tag, sort = TRUE)

fetch_artist_toptags("Soul Glo")
```
