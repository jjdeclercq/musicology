...
jp_wl %>% count(item2, name = "loss"), by = c("item" = "item2")) %>%
  ...
  mutate(ability = round(ability, 4)) %>% filter(matches >50) %>% 
  arrange(desc(ability)) %>% mutate(a_rank = 1:n()) %>% 
...
jpa %>% left_join(., 
                  jp %>% count(order),
                  by = c( "order")) %>% 
  mutate(n = replace_na(n, 0), p = 1-(n/(1+max(n))), p = p^2, q = rank(p), qr = q/max(q)) %>% arrange(qr)
